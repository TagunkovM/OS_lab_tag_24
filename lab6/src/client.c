...
int main(int argc, char **argv) {
  uint64_t k = -1;
  uint64_t mod = -1;
  char servers[255] = {'\0'}; // TODO: explain why 255(Размер 255 выбран, чтобы обеспечить достаточную длину для хранения стандартного пути к файлу в Unix-подобных системах. Это значение часто используется, так как оно соответствует максимальной длине имени файла в большинстве файловых систем.)
  FILE *a;
  ...
  while (!feof(a)) {
    ...
    fscanf(a, "%s\n", tmp);
    char *marker = strtok(tmp, ":");//strtok выполняет поиск лексем в строке string.
  ...
  for (int i = 0; i < servers_num; i++) {
    ...
    server.sin_port = htons(to[i].port);//htons преобразует u_short из узла в порядок байтов сети TCP/IP, который является большим порядком байтов
